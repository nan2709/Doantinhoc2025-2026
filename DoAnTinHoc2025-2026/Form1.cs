...
namespace DoAnTinHoc2025_2026
{
    ...
    "Tổng lá lẻ"
            ...
            comboBoxDuplicate.Items.AddRange(new string[]
{
    "Nút trùng nhiều nhất",
    "Nút trùng ít nhất",
    "Xuất toàn bộ nhóm trùng"

});
            comboBoxDuplicate.SelectedIndex = 0;
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (selected)
            {
                ...
                case "Xuất lá":
                    {
                        var list = avl.GetLeafValues(avl.Root);
                        MessageBox.Show("Các node lá:\n" + string.Join(", ", list));
                        break;
                    }

                case "Xuất lá chẵn":
                    {
                        var list = avl.GetLeafEvenValues(avl.Root);
                        MessageBox.Show("Các node lá chẵn:\n" + string.Join(", ", list));
                        break;
                    }

                case "Xuất lá lẻ":
                    {
                        var list = avl.GetLeafOddValues(avl.Root);
                        MessageBox.Show("Các node lá lẻ:\n" + string.Join(", ", list));
                        break;
                    }

                case "Tổng lá":
                    {
                        MessageBox.Show("Tổng các node lá = " + avl.SumLeaf(avl.Root));
                        break;
                    }

                case "Tổng lá chẵn":
                    {
                        MessageBox.Show("Tổng các node lá chẵn = " + avl.SumLeafEven(avl.Root));
                        break;
                    }

                case "Tổng lá lẻ":
                    {
                        MessageBox.Show("Tổng các node lá lẻ = " + avl.SumLeafOdd(avl.Root));
                        break;
                    }
        ...
        private void btnCayNguyenThuy_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count == 0)
            {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                double math;
                if (!double.TryParse(row.Cells[5].Value?.ToString(), out math))
                    continue;
            ...
            avl = new AVLTree();
            foreach (var s in students)
                avl.Root = avl.InsertPrimitive(avl.Root, s);
            ...
            MessageBox.Show("✅ Đã tạo cây nguyên thủy (không trùng MathScore)");
        }
        ...
        private void comboBoxDuplicate_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (avl == null || avl.Root == null)
            {
                MessageBox.Show("Vui lòng tạo cây trước!");
                return;
            }

            string selected = comboBoxDuplicate.SelectedItem.ToString();

            switch (selected)
            {
                case "Nút trùng nhiều nhất":
                    {
                        var result = avl.FindMostDuplicateNode(avl.Root);
                        MessageBox.Show(
                            $"Giá trị trùng nhiều nhất: {result.Key}\n" +
                            $"Số lần xuất hiện: {result.Count}"
                        );
                        break;
                    }

                case "Nút trùng ít nhất":
                    {
                        var result = avl.FindLeastDuplicateNode(avl.Root);
                        MessageBox.Show(
                            $"Giá trị trùng ít nhất: {result.Key}\n" +
                            $"Số lần xuất hiện: {result.Count}"
                        );
                        break;
                    }
        ...
        {
            avl = new AVLTree();
            avl.BuildAVLTreeAllowDuplicate(currentStudents);

            MessageBox.Show("✅ Đã tạo cây AVL mới (cho phép node trùng).\nCó thể dùng combobox thống kê trùng.");
        }
    }
}
